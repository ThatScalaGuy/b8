...
/** The container over a codec that has no opinions, so that anything that goes
  ...
  *
  * The array the vector is compared against is taken from `Encoder.encode`
  * directly rather than through `b8.array`. Not because the two imports would
  * clash here — this file is inside `b8.vector`, where the extensions are
  ...
  * how this suite stays out of reach of it. `ImportSuite` is where the
  * ambiguity a user actually meets is pinned down.
  */
class RoundTripSuite extends ScalaCheckSuite:

  private val genChunk: Gen[ByteVector] =
    Gen
      .choose(0, 8)
      .flatMap(n =>
        Gen
          .listOfN(n, Gen.choose(Byte.MinValue, Byte.MaxValue))
          .map(bs => ByteVector.view(bs.toArray))
      )

  property("the vector holds exactly the bytes the encoder produced") {
    forAll { (s: String) =>
      s.encode[Utf8] == ByteVector(Encoder[String, Utf8].encode(s))
    }
  }

  property("the vector is exact-size") {
    forAll { (s: String) =>
      s.encode[Utf8].size == Encoder[String, Utf8].encode(s).length.toLong
    }
  }
  ...
  property("a slice of a concatenation decodes to the same value") {
    forAll(Gen.alphaNumStr, genChunk, genChunk) {
      (s: String, prefix: ByteVector, suffix: ByteVector) =>
        val framed = prefix ++ s.encode[Utf8] ++ suffix
        framed
          .drop(prefix.size)
  ...
  property("a thread-local pool changes neither the bytes nor the round trip") {
    ...
    forAll { (s: String) =>
      val bytes = s.encode[Utf8]
      bytes == ByteVector(Encoder[String, Utf8].encode(s)) &&
  ...
  property("encodeTo writes the same bytes into a sink the caller brought") {
    forAll { (s: String) =>
      ...
      ByteVector.view(sink.result()) == s.encode[Utf8]
    }
  }
